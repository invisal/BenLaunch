...
const makeStore = (): WidgetStore =>
  new WidgetStore(new ExtensionStorage(file(), "ext:widget"));
...
test("missing, corrupt, and malformed storage all yield an empty list", () => {
  ...
  writeFileSync(
    file(),
    JSON.stringify({ version: 1, savedAt: 0, data: { widgets: "nope" } }),
  );
  assert.deepEqual(makeStore().list(), []);
  ...
  writeFileSync(
    file(),
    JSON.stringify({ version: 999, savedAt: 0, data: { widgets: [] } }),
  );
  assert.deepEqual(makeStore().list(), []);
});
