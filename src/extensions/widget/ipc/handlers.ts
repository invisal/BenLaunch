...
export function registerWidgetIpc(
  ...
  ipc.handle(WIDGET_CHANNELS.save, (_event, draft: WidgetDraft) =>
    store.save(draft),
  );
  ...
  ipc.handle(
    WIDGET_CHANNELS.setExposed,
    (_event, id: string, exposed: boolean) => {
      store.setExposed(id, exposed);
    },
  );
  ipc.handle(WIDGET_CHANNELS.test, (_event, code: string) =>
    runner.runOnce(code),
  );
}
