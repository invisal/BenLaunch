...
import { WindowLayoutStore } from "./store.ts";
...
beforeEach(() => {
  dir = mkdtempSync(join(tmpdir(), "window-layout-"));
});
...
const file = (): string => join(dir, "window.json");
const makeStore = (): WindowLayoutStore =>
  new WindowLayoutStore(new ExtensionStorage(file(), "ext:window"));
...
test("missing, corrupt, and malformed storage all yield an empty list", () => {
  ...
  // Right wrapper, wrong `layouts` shape → the store's own guard rejects it.
  writeFileSync(
    file(),
    JSON.stringify({ version: 1, savedAt: 0, data: { layouts: "nope" } }),
  );
  assert.deepEqual(makeStore().list(), []);
  ...
  writeFileSync(
    file(),
    JSON.stringify({ version: 999, savedAt: 0, data: { layouts: [] } }),
  );
  assert.deepEqual(makeStore().list(), []);
});
