...
function edgeSpan(size: number, position: EdgePosition): FractionSpan {
  const start =
    position === "first" ? 0 : position === "last" ? 1 - size : (1 - size) / 2;
  return { start, size };
}
...
const QUARTER_INDEX: Record<QuarterPosition, number> = {
  first: 0,
  second: 1,
  third: 2,
  last: 3,
...
const GRID_REGIONS = {
  ...
  "first-three-fourths": {
    col: edgeSpan(THREE_FOURTHS, "first"),
    row: FULL_SPAN,
  },
  "center-three-fourths": {
    col: edgeSpan(THREE_FOURTHS, "center"),
    row: FULL_SPAN,
  },
  "last-three-fourths": {
    col: edgeSpan(THREE_FOURTHS, "last"),
    row: FULL_SPAN,
  },
  ...
  "top-three-fourths": {
    col: FULL_SPAN,
    row: edgeSpan(THREE_FOURTHS, "first"),
  },
  "bottom-three-fourths": {
    col: FULL_SPAN,
    row: edgeSpan(THREE_FOURTHS, "last"),
  },

  "top-left-sixth": {
    col: edgeSpan(THIRD, "first"),
    row: edgeSpan(HALF, "first"),
  },
  "top-center-sixth": {
    col: edgeSpan(THIRD, "center"),
    row: edgeSpan(HALF, "first"),
  },
  "top-right-sixth": {
    col: edgeSpan(THIRD, "last"),
    row: edgeSpan(HALF, "first"),
  },
  "bottom-left-sixth": {
    col: edgeSpan(THIRD, "first"),
    row: edgeSpan(HALF, "last"),
  },
  "bottom-center-sixth": {
    col: edgeSpan(THIRD, "center"),
    row: edgeSpan(HALF, "last"),
  ...
  "top-first-fourth": {
    col: quarterSpan("first"),
    row: edgeSpan(HALF, "first"),
  },
  "top-second-fourth": {
    col: quarterSpan("second"),
    row: edgeSpan(HALF, "first"),
  },
  "top-third-fourth": {
    col: quarterSpan("third"),
    row: edgeSpan(HALF, "first"),
  },
  ...
  "bottom-last-fourth": {
    col: quarterSpan("last"),
    row: edgeSpan(HALF, "last"),
  },

  "top-center-two-thirds": {
    col: edgeSpan(TWO_THIRDS, "center"),
    row: edgeSpan(HALF, "first"),
  },
 ...
 * authoritative list `WindowExtension` builds its commands from, so a
 ...
 * `WindowExtension` can derive each command's icon from the same
...
export function regionSpan(
  region: SnapRegion,
): { col: FractionSpan; row: FractionSpan } | null {
  return (
    (
      GRID_REGIONS as Record<
        string,
        { col: FractionSpan; row: FractionSpan } | undefined
      >
    )[region] ?? null
  );
}
  ...
  switch (region as SpecialRegion) {
    case "center":
      return round(
        centeredIn(
          workArea,
          currentRect
            ? clampSize(currentRect.width, width)
            : width * DEFAULT_CENTER_FRACTION,
          currentRect
            ? clampSize(currentRect.height, height)
            : height * DEFAULT_CENTER_FRACTION,
        ),
      );
    case "center-half":
    ...
    case "almost-maximize":
      return round(
        centeredIn(
          workArea,
          width * ALMOST_MAXIMIZE_FRACTION,
          height * ALMOST_MAXIMIZE_FRACTION,
        ),
      );
    case "maximize":
    ...
    default:
      ...
      // a caller building an id from a string template (`WindowExtension`'s
...
export function computeCustomRect(
  ...
  const width =
    layout.widthFraction != null
      ...
      : clampSize(
          currentRect?.width ?? workArea.width * DEFAULT_CENTER_FRACTION,
          workArea.width,
        );
  const height =
    layout.heightFraction != null
      ...
      : clampSize(
          currentRect?.height ?? workArea.height * DEFAULT_CENTER_FRACTION,
          workArea.height,
        );
...
export function mapRectToDisplay(
  ...
  const relX =
    fromWorkArea.width === 0
      ? 0
      : (rect.x - fromWorkArea.x) / fromWorkArea.width;
  const relY =
    fromWorkArea.height === 0
      ? 0
      : (rect.y - fromWorkArea.y) / fromWorkArea.height;
  const relW = fromWorkArea.width === 0 ? 1 : rect.width / fromWorkArea.width;
  const relH =
    fromWorkArea.height === 0 ? 1 : rect.height / fromWorkArea.height;
  ...
  const width = Math.round(
    clampSize(relW * toWorkArea.width, toWorkArea.width),
  );
  const height = Math.round(
    clampSize(relH * toWorkArea.height, toWorkArea.height),
  );
  ...
  return {
    x: Math.round(
      Math.min(
        Math.max(toWorkArea.x + relX * toWorkArea.width, toWorkArea.x),
        maxX,
      ),
    ),
    y: Math.round(
      Math.min(
        Math.max(toWorkArea.y + relY * toWorkArea.height, toWorkArea.y),
        maxY,
      ),
    ),
    width,
