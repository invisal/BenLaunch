...
test('hasPlaceholder recognizes {query}, {argument}, {arg}, {} and {argument name="…"}', () => {
  ...
  assert.equal(
    hasPlaceholder('https://x.com/s?q={argument name="query"}'),
    true,
  );
...
test("expandDynamic fills clipboard, uuid, date and time", () => {
  ...
  assert.equal(
    expandDynamic("https://x.com/{date}", { now }),
    "https://x.com/2026-09-02",
  );
  assert.equal(
    expandDynamic("https://x.com/{time}", { now }),
    "https://x.com/14:07",
  );
...
test("add() appends a normalized entry with a unique generated id", () => {
  ...
  assert.equal(
    second.id,
    "hacker-news-2",
    "id collision gets a numeric suffix",
  );
...
test("normalizeTags trims, lowercases, drops blanks, de-duplicates", () => {
  assert.deepEqual(normalizeTags([" Work ", "work", "DEV", "", "  "]), [
    "work",
    "dev",
  ]);
...
test("update() rejects an unknown id and an invalid draft", () => {
  ...
  store.reload();
  assert.throws(
    () => store.update("nope", { name: "X", link: "https://x.com" }),
    /no longer exists/i,
  );

  store.add({ name: "X", link: "https://x.com" });
  assert.throws(
    () => store.update("x", { name: "", link: "https://x.com" }),
    /name/i,
  );
});
