/**
 * On-disk cache for the last successful `AppsWorkerResult`, so a launch can serve
 * the previously-known app list instantly instead of waiting for apps-worter.ts to
 * finish its first (multi-second, cold) run. actions.ts serves this stale list on
 * startup and re-runs the worker in the background to refresh it.
 *
 * Unlike icon-cache.ts this runs in the main process, so it owns the file path via
 * Electron's `app`. The cache is purely advisory: every failure is swallowed (with
 * a `[apps-cache]` prefix) and leaves the caller to fall back to a fresh worker run.
 */
