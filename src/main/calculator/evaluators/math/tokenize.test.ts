...
test("classifies functions vs units vs constants", () => {
  assert.equal(
    shape(tokenize("sqrt(144)")),
    "sqrt:function (:paren 144:number ):paren",
  );
  assert.equal(shape(tokenize("2 * pi")), "2:number *:operator pi:constant");
  assert.equal(
    shape(tokenize("128 GB to MB")),
    "128:number GB:unit to:operator MB:unit",
  );
  assert.equal(
    shape(tokenize("10 cm in mm")),
    "10:number cm:unit in:operator mm:unit",
  );
});
...
test("decimals and exponents are single number tokens", () => {
  ...
  assert.equal(
    shape(tokenize("1.5e3 * 2")),
    "1.5e3:number *:operator 2:number",
  );
});
