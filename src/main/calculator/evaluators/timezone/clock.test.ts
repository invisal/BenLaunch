...
test("what time is it in <place>", () => {
  assert.equal(
    resolveClock("what time is it in Berlin", NOW, UTC)?.value,
    "14:00 · GMT+2",
  );
});
...
test('what time in <place> (no "is it")', () => {
  assert.equal(
    resolveClock("what time in Berlin", NOW, UTC)?.value,
    "14:00 · GMT+2",
  );
});
...
test('the trailing-"time" shape also lists a multi-zone country', () => {
  assert.equal(
    resolveClock("united states time", NOW, UTC)?.value,
    resolveClock("time in united states", NOW, UTC)?.value,
  );
});

test("a country with zero zones of its own stays unresolved", () => {
  assert.equal(
    resolveClock("time in heard island and mcdonald islands", NOW, UTC),
    null,
  );
});
...
test("a relative offset shifts the clock — either side of the place", () => {
  ...
  assert.equal(
    resolveClock("time in 90 min in London", NOW, UTC)?.value,
    "14:30 · GMT+1",
  );
  assert.equal(
    resolveClock("time in an hour in Berlin", NOW, UTC)?.value,
    "15:00 · GMT+2",
  );
});
...
test('a relative offset can point at the past with "ago"', () => {
  assert.equal(
    resolveClock("time in Tokyo 3 hours ago", NOW, UTC)?.value,
    "18:00 · GMT+9",
  );
});
...
test('an arithmetic offset ("+ 6 hours") on the place also shifts the clock', () => {
  // NOW is 15 Jun 12:00 UTC → Tokyo 21:00; +6h lands on the 16th there.
  assert.equal(
    resolveClock("time in Tokyo + 6 hours", NOW, UTC)?.value,
    "03:00 Tue · GMT+9",
  );
  assert.equal(
    resolveClock("time in Tokyo + 6 hour", NOW, UTC)?.value,
    "03:00 Tue · GMT+9",
  );
  assert.equal(
    resolveClock("time in Tokyo - 2h", NOW, UTC)?.value,
    "19:00 · GMT+9",
  );
  assert.equal(
    resolveClock("time in London + 90 min", NOW, UTC)?.value,
    "14:30 · GMT+1",
  );
  // bare number ⇒ hours
  assert.equal(
    resolveClock("time in Tokyo + 6", NOW, UTC)?.value,
    "03:00 Tue · GMT+9",
  );
  assert.equal(
    resolveClock("time in Tokyo + 6", NOW, UTC)?.expression,
    "time in Tokyo + 6",
  );
});
...
test('"time in N hours" with no place — the viewer\'s own clock, then', () => {
  ...
  assert.deepEqual(
    { ...resolveClock("time in 4 hours", NOW, UTC) },
    {
...
test("appends the weekday when the day differs from the local one", () => {
  ...
  assert.match(
    resolveClock("time in Auckland", NOW, UTC)?.value ?? "",
    /^\d{2}:\d{2} \w{3} · GMT\+12$/,
  );
});
