...
## 10. Not claimed — nothing shown, query falls through

| #    | Input                | Why                                       | ✓   |
| ---- | -------------------- | ----------------------------------------- | --- |
| 10.1 | `` (empty)           | nothing to parse                          |     |
| 10.2 | `chrome`, `photoshop`| no date-ish token                         |     |
| 10.3 | `5 + 3`, `2 + 2`     | plain math                                 |     |
| 10.4 | `10 usd to eur`      | currency, claimed earlier in the chain    |     |
| 10.5 | `5 in ft`            | unit conversion                            |     |
| 10.6 | `today's news`, `monday.com` | a date word inside another string — must not hijack |  |
| 10.7 | `call at 5pm`, `1:1 meeting`, `august occasion` | a time/month word but no `±` operator or operand |  |
| 10.8 | `flight to paris`, `message to bob` | `to` but neither side is a date |  |
