...
test("formatDuration: explicit unit", () => {
  assert.equal(
    formatDuration(90 * 24 * 60 * 60 * 1000, "days").value,
    "90 days",
  );
...
test("formatDuration: auto-picks a unit", () => {
  ...
  assert.equal(
    formatDuration(100 * 24 * 60 * 60 * 1000).value.endsWith("weeks"),
    true,
  );
  assert.equal(
    formatDuration(300 * 24 * 60 * 60 * 1000).value.endsWith("months"),
    true,
  );
});

test("formatDuration: sub-day auto spans resolve to the minute", () => {
  assert.deepEqual(formatDuration(45 * 60 * 1000), {
    value: "45 minutes",
    rawValue: "45",
  });
  assert.deepEqual(formatDuration(60 * 60 * 1000), {
    value: "1 hour",
    rawValue: "60",
  });
