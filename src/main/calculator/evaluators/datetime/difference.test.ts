...
test('a trailing "in <unit>" forces the result unit', () => {
  assert.equal(
    resolveDifference("1988-12-08 to today in days", NOW)?.value,
    "13785 days",
  );
  assert.equal(
    resolveDifference("1988-12-08 to today in day", NOW)?.value,
    "13785 days",
  );
  assert.equal(
    resolveDifference("1988-12-08 to today in weeks", NOW)?.value,
    "1969 weeks",
  );
  // a leading unit still wins over a trailing one
  assert.equal(
    resolveDifference("days between 1 Jan and 1 Apr in weeks", NOW)?.value,
    "90 days",
  );
});
...
test('sub-day span is shown to the minute, not rounded to "1 day"', () => {
  ...
  assert.equal(
    resolveDifference("now until 10:30AM", NOW)?.value,
    "30 minutes",
  );
