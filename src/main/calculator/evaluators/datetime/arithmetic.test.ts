...
test("bare number after a clock time ⇒ hours, and stays a clock time", () => {
  assert.deepEqual(
    { ...resolveArithmetic("3:45pm + 5", NOW) },
    {
      expression: "3:45pm + 5",
      value: "8:45 PM",
      rawValue: "20:45",
...
test("explicit unit tokens, long and abbreviated", () => {
  assert.equal(
    resolveArithmetic("2026-01-15 + 3 weeks", NOW)?.value,
    "Thu, Feb 5",
  );
...
test("month/year math is calendar-correct (date-fns)", () => {
  ...
  assert.equal(
    resolveArithmetic("2026-01-31 + 1 month", NOW)?.value,
    "Sat, Feb 28",
  );
  assert.equal(
    resolveArithmetic("2026-01-15 + 1 year", NOW)?.value,
    "Fri, Jan 15, 2027",
  );
});
...
test("a clock-time result that crosses midnight is flagged", () => {
  assert.equal(
    resolveArithmetic("11pm + 3h", NOW)?.value,
    "2:00 AM (next day)",
  );
  assert.equal(
    resolveArithmetic("1am - 2h", NOW)?.value,
    "11:00 PM (prev day)",
  );
});
...
test('"now + N" keeps the calendar date (now is a fixed instant, not a bare clock)', () => {
  assert.equal(
    resolveArithmetic("now + 90 min", NOW)?.value,
    "Sat, Sep 5, 11:30 AM",
  );
});
